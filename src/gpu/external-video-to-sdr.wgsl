...
fn opaque_browser_rgb_preview(rgb: vec3<f32>) -> vec3<f32> {
  let linear = srgb_to_linear(rgb);
  let luma = dot(linear, vec3<f32>(0.2126, 0.7152, 0.0722));
  let softened = aces_filmic(linear * vec3<f32>(1.15));
  let preview = mix(vec3<f32>(luma), softened, 0.92);
  return linear_to_srgb(preview);
}

@fragment
fn fragment_main(input: VertexOutput) -> @location(0) vec4<f32> {
  let sampled = textureSampleBaseClampToEdge(videoTexture, videoSampler, input.uv);
  return vec4<f32>(opaque_browser_rgb_preview(sampled.rgb), sampled.a);
}
