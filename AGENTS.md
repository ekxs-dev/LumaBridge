...
- `/bench` also has a "Fast WebCodecs preview" button. It decodes parsed HEVC samples through WebCodecs and draws opaque `VideoFrame`s directly to the SDR preview canvas with `drawImage()`. This is the normal-speed/browser-hardware preview path, but it is not the strict DV P5 raw path because Chrome currently reports `VideoFrame.format === null` for the user's HEVC sample and does not expose I420P10 planes via `copyTo()`.
