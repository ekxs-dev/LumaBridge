...
- `/bench` has a "Fast WebGPU SDR preview" button. It decodes through WebCodecs, imports each opaque `VideoFrame` via `GPUDevice.importExternalTexture()`, then renders a fullscreen WebGPU shader from `src/gpu/external-video-to-sdr.wgsl` into a dedicated external-texture canvas. This is the fastest interactive WebGPU entry available when raw `I420P10` is unavailable, but it starts from browser-converted RGB and may already include YUV/RGB/color-space conversion, clamping, or DV-incorrect interpretation. The shader currently defaults to a diagnostic recovery pass: invert the browser's visible BT.709-ish RGB into pseudo YUV, reinterpret those channels as DV P5 IPT/PQ base data, then tone-shape to SDR. The `external recovery` selector can switch matrix/range/channel assumptions (`opaque-rgb`, 709/2020, full/limited, UV swap, U/V/UV flip) so manual testing can identify the least-wrong opaque path. Treat it as approximate interaction/debug preview only, not reference SDR.
- The fast WebCodecs canvas preview and fast WebGPU external-texture preview start from the current `/bench` seconds control. They decode from the nearest previous sync sample, skip pre-roll frames before the requested timestamp, and report both the requested time and the first drawn timestamp. The fast WebGPU external-texture preview is paced by `VideoFrame.timestamp` and defaults to up to 720 frames / 30 seconds, but actual duration is limited by the parsed sample window. Large MKV inputs are still prefix-parsed, so the current user sample can only preview the samples available in the 16 MB prefix until streaming Matroska demux is implemented.
- `/bench` has a "Realtime preview" control for the currently feasible browser path. Today it runs a wall-clock ffmpeg.wasm fallback loop with target FPS clamped to 0.25-4fps and defaulting to 0.25fps for 4K safety, skips ahead when decode/render is slower than target, and records effective FPS/dropped-frame estimates. This is useful for interactive visual tracking, but it is not strict 24fps DV playback. Strict realtime still requires WebCodecs streaming decode plus `VideoFrame.copyTo()` I420P10 frames.
...
- 8.5s manual diagnostic on `/path/to/input.mkv`: native FFmpeg/libplacebo reference is green, fast external-texture recovery modes are white/cyan because Chrome exposes only opaque bt709-ish `VideoFrame`s, while the ffmpeg.wasm raw I420P10 WebGPU diagnostics produce a green logo but still diverge from libplacebo in edge/channel details. Artifacts are local under `artifacts/external-recovery-8_5s-chrome/` and `artifacts/raw-preview-8_5s-chrome/` and should stay untracked unless explicitly requested.
...
- [x] Add libplacebo-style RGB softclip to SDR diagnostic output before BT.1886 encoding.
...
- [x] Normalize HEVC profile compatibility flags for WebCodecs codec strings and probe relaxed `hev1`/`hvc1` candidates before ffmpeg.wasm fallback.
- [x] Add raw I420P10 Y/U/V frame statistics and CPU fallback when WebGPU returns a black frame for non-black raw data.
- [x] Add fast WebCodecs opaque-frame preview for normal-speed decode diagnostics.
